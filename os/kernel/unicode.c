...
/* CP437 code position -> Unicode (Wikipedia Code page 437). */
static const uint32_t cp437_unicode[256] = {
...
uint32_t utf8_next(const char** pp)
{
    ...
    if ((c0 & 0xE0u) == 0xC0u && p[1] != 0 && (p[1] & 0xC0u) == 0x80u) {
        uint32_t cp = ((uint32_t)(c0 & 0x1Fu) << 6) | (uint32_t)(p[1] & 0x3Fu);
        if (cp < 0x80u) {
            *pp += 1;
            return 0xFFFDu;
        }
    ...
    if ((c0 & 0xF0u) == 0xE0u && p[1] != 0 && p[2] != 0 && (p[1] & 0xC0u) == 0x80u
        && (p[2] & 0xC0u) == 0x80u) {
        uint32_t cp = ((uint32_t)(c0 & 0x0Fu) << 12) | ((uint32_t)(p[1] & 0x3Fu) << 6)
            | (uint32_t)(p[2] & 0x3Fu);
        if (cp < 0x800u) {
            *pp += 1;
            return 0xFFFDu;
    ...
    if ((c0 & 0xF8u) == 0xF0u && p[1] != 0 && p[2] != 0 && p[3] != 0
        && (p[1] & 0xC0u) == 0x80u && (p[2] & 0xC0u) == 0x80u && (p[3] & 0xC0u) == 0x80u) {
        uint32_t cp = ((uint32_t)(c0 & 0x07u) << 18) | ((uint32_t)(p[1] & 0x3Fu) << 12)
...
static void put_cp437_glyph(void (*put_byte)(uint8_t b, void* user), void* user, uint32_t cp)
{
    if (cp < 0x80u) {
        put_byte((uint8_t)cp, user);
        return;
    }
    put_byte(unicode_to_cp437(cp), user);
}
    ...
    void (*put_byte)(uint8_t b, void* user), void* user, const char* utf8)
{
    ...
    for (;;) {
        ...
        put_cp437_glyph(put_byte, user, cp);
    }
}

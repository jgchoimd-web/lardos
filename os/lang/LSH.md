...
- `megaclip status|list|mode stack|single|order|push text|file path|pull slot|write slot file` - manage moving clipboard history. `pinclip list|set slot text|pull slot|write slot file|clear slot|reload` manages fixed shortcut slots that stay put when history moves. GUI shortcuts: `Ctrl+Y`, `Ctrl+P`, `Ctrl+Space` then `1..9`/`0`, and `Ctrl+Space` then `P` then `1..9`/`0` for fixed slots.

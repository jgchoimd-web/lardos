...
- `dos on|off|status|help|map|log|test` - enter L-DOS mode, a native DOS-style shell layer with case-insensitive `DIR`, `TYPE`, `COPY`, `DEL`, `REN`, `MD`, `RD`, `CD`, `CLS`, `VER`, `SET`, `ECHO`, `MEM`, and `EXIT`.
